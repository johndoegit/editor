	...
	function diff_linesToCharsMunge($text, &$lineArray, &$lineHash) {
		...
		while ($lineEnd < mb_strlen($text) - 1) {
			...
			if ( isset($lineHash[$line]) ) {
				$chars .= codiad_codiad_mb_chr($lineHash[$line]);
			} else {
				$chars .= codiad_codiad_mb_chr($lineArrayLength);
...
@				$charMatch = $s[ $text[$j -1] ];
	...
	function patch_addPadding(&$patches){
		...
		for ($x = 1; $x <= $paddingLength; $x++) {
			$nullPadding .= codiad_codiad_mb_chr($x);
		}
...
function charCodeAt($str, $pos) {
	return codiad_codiad_mb_ord(mb_substr($str, $pos, 1));
}
function codiad_codiad_mb_ord($v) {
	$k = mb_convert_encoding($v, 'UCS-2LE', 'UTF-8'); 
	$k1 = ord(substr($k, 0, 1)); 
	$k2 = ord(substr($k, 1, 1)); 
	return $k2 * 256 + $k1; 
}
function codiad_codiad_mb_chr($num){
